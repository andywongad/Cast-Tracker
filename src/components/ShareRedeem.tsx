  ...
  return (
    <Sheet onClose={closeShareSheet} label="Share" sheetStyle={{ maxHeight: 'none', padding: '22px 18px 28px' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 16 }}>
          <div style={{ width: 48, height: 48, borderRadius: 12, flex: 'none', backgroundColor: 'var(--surface)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 16, fontWeight: 800, color: 'rgba(255,255,255,0.85)', ...bgStyle(shareSheet.photo) }}>
        ...
        <div style={{ display: 'flex', gap: 8, marginBottom: 14 }}>
          ...
          <button onClick={copy} style={{ flex: 'none', height: 46, padding: '0 18px', border: 'none', borderRadius: 12, background: 'var(--accent)', color: 'var(--accent-text)', fontSize: 13, fontWeight: 700, cursor: 'pointer' }}>{copied ? 'Copied!' : 'Copy'}</button>
        </div>
        ...
        <div style={{ fontSize: 14, color: 'var(--text-faint)', lineHeight: 1.5 }}>Whoever opens this can add their own copy to their tracker &mdash; and edit it freely from there. (Sharing works within this browser today; cross-device delivery needs a backend.)</div>
    </Sheet>
  );
}
